def evalrpn(tokens):
    ...
    for ele in range(0, len(tokens)):
        # print digList
        try:
            digList.append(int(tokens[ele]))

        except ValueError:
            if tokens[ele] == '+':
                res = int(float(digList.pop(-2)) + float(digList.pop()))
                digList.append(res)
